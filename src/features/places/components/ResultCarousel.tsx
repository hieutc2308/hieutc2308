...
function buildCarouselList(suggestions: PlaceSuggestion[]) {
...
function CarouselCards({ suggestions }: { suggestions: PlaceSuggestion[] }) {
  const [cardSize, setCardSize] = useState(320);
  const [list, setList] = useState(() => buildCarouselList(suggestions));
...
export function ResultCarousel({ suggestions, loading, error, hasSearched, searchCount }: ResultCarouselProps) {
  ...
  if (!hasSearched || suggestions.length === 0) return null;

  return <CarouselCards key={searchCount} suggestions={suggestions} />;
}
